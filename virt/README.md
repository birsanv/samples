...
Policy      | Description 
-------------------------------------------| ----------- 
[acm-dr-virt-install](./policies/acm-dr-virt-install.yaml)                       | Installs OADP and configures the DataProtectionApplication resource on the cluster where this policy is placed. If the cluster is a hub cluster, it will just validate that OADP is installed in the open-cluster-management-backup namespace and validates that the DPA exists and has the required configuration. This is because on the hub OADP should be installed by the backup chart using the MCH backup option.
