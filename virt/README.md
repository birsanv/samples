...
An example of such configuration is available [here](./acm-virt-config14.yaml) and [here](./acm-virt-config-13.yaml).
...
- The user wants to enable virt backup on a managed cluster `cls1`:
  ...
  3. The user applies on ManagedCluster `cls` this label : `acm-virt-config=acm-virt-config13.yaml` . This will result in the Policies being placed on this cluster. 
